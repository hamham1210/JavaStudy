...
public class StudentCollectionTest {
	public static void main(String[] args) {
		...
		List<Student> downnerList = totalList.stream()
				.sorted(Comparator
						.comparing(Student::getScore)
						.reversed())
						.collect(Collectors.toList()); 
		
		for(Student s : downnerList) {
			System.out.println(s.getName() +","+s.getScore());
		}
		...
		Map<String, Integer> map = totalList.stream().sorted(Comparator.comparing(Student::getScore).reversed())
