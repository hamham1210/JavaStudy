...
public class MyBatisTestApp05 {
	...
	public static void main(String[] args) throws Exception{
		 ...
		 * session.insert("UserMapper05.addUser",user)); //1. session.commit();
		 * 
		 * System.out.println("\n");
		 * 
		 * //2. User05.addUser Test :: users table age/grade/redDate 입력값 확인할것 : OK
		 * user.setUserId("user05");//아이디를 변경...pk여서 새로운 user가 들어간다.
		 * System.out.println(":: 2. addUser(INSERT)  ? " +
		 * session.insert("UserMapper05.addUser",user));//1 session.commit();
		 * System.out.println("\n");
		 * 
		 * //3. User05.uadateUser Test :: users table age/redDate 입력값 확인할것 : OK // :: 주몽
		 * ==> 온달 수정 user.setUserName("온달");//user05의 이름 변경.
		 * System.out.println(":: 3. update(UPDATE)  ? " +
		 * session.update("UserMapper05.updateUser",user));//1 session.commit();
		 */
		
		//4. user05 아이디를 가진 사람을 삭제하는 단위 테스트를 작성
		System.out.println(":: 5. delete(delete)  ? "
				+ session.delete("UserMapper05.removeUser",user.getUserId()));
		session.commit();
	}//end of main
}//end of class
