...
public class BankTest {		
	...
	/*
	 * 스캐너로 받을 값이 많을 때는 직접 객체에 넣지말고 함수를 만들어서 넣어줘야한다.
	 */
	private static void addCustomer() {
		...
		System.out.println(name+"님 계좌 개설 되었습니다.");
	...
	private static void createAccount() {
		...
		if(BankService.getInstance().getCustomer(ssn).getName()!= "") {
			...
			System.out.println("회원이 존재하지 않습니다... 회원가입부터 진행해주세요.");
			return;
		}
	...
	private static void showCustomer() {//특정회원에 대한 정보를 출력..
		...
		if(BankService.getInstance().getCustomer(ssn).getName() != "") {
			...
			System.out.println("존재하는 회원이 없어서 되돌아갑니다.");
			return;
		}
